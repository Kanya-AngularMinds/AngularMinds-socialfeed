...
import { Avatar, Box, Button, Card, Container, Divider, FormControl, FormControlLabel, Grid, IconButton, Menu, MenuItem, Modal, Paper, Radio, RadioGroup, TextareaAutosize, TextField, Typography } from '@mui/material';
...
function Editprofile({openEdit,setOpenEdit}:booleanProps) {
  ...
  const[profilepic,setProfilepic] = useState({file:[],filepreview:[]} as any);
  ...
  const handleImg = (e: any) => { 
    ...
    patch(`http://localhost:8080/users/profilePicture/${userObj._id}`,formData).then((response) => {
  ...
  let editedObj ={
   ...
   patch(`http://localhost:8080/users/${userObj._id}`,editedObj).then((response) =>{
     console.log(response);
     localStorage.setItem("currentUser", JSON.stringify(response));
   
     let msg = (
       <p style={{ fontSize: 15 }}>Profile updated successfully </p>
     );
     toast.success(msg, {
       position: toast.POSITION.TOP_RIGHT,
       icon: CheckCircleOutlineIcon,
      ...
      <IconButton onClick={() =>setOpenEdit(false)}>
        ...
        <Avatar
         ...
         <img src={userObj ? userObj.profilePicture:null} />
          ...
          <IconButton 
            edge="end" style={{marginLeft:"79px",top:"-9%"}} > 
          
            <AddAPhotoRoundedIcon sx={{color:"#1890FF",backgroundColor:"white",padding:"7px",borderRadius:"50%"}} />
 ...
 <Grid container spacing={2}  style={{marginTop:"-120px"}}>
      ...
      <Grid item xs={12}>
        <TextareaAutosize
          ...
          style={{ width: 500, borderColor: "light" }}
        />
      </Grid>
      <Grid item xs={12}>
        ...
        <FormControl >
          ...
          >
            <FormControlLabel
              ...
              control={<Radio required={true} checked={gender === "female"}   />}
              label="Female"
            />
            <FormControlLabel
              value="male"
      ...
      <Grid item xs={12}>
        <Button variant="contained" fullWidth className="btn" style={{width:"100%"}} onClick={()=> handleEdit()}>Save Profile</Button>
      </Grid>
          ...
          >
            ...
            >
              {" "}
              <input type="file" id="upload-photo" style={{display:"none"}} onChange={(e:any) => handleImg(e)} name="profilePicture"/>
